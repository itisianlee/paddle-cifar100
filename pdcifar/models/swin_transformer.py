# --------------------------------------------------------
# implement Swin Transformer with paddle
...
from .transformer import DropPath, trunc_norm_, Identity
...
class WindowAttention(nn.Layer):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        paddle.assign(trunc_norm_(self.relative_position_bias_table.shape, std=0.02), self.relative_position_bias_table)
    ...
    def forward(self, x, mask=None):
        ...
        ).reshape((self.window_size[0] * self.window_size[1], self.window_size[0] * self.window_size[1], -1))  # Wh*Ww,Wh*Ww,nH
...
class SwinTransformer(nn.Layer):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        if self.ape:
            attr = ParamAttr(initializer=nn.initializer.Constant(0))
            self.absolute_pos_embed = self.create_parameter(shape=(1, num_patches, embed_dim), attr=attr)
            paddle.assign(trunc_norm_(self.absolute_pos_embed.shape, std=0.02), self.absolute_pos_embed)
