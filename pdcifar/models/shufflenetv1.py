...
from .utils.init import kaiming_normal_
...
def conv3x3(in_channels, out_channels, stride=1, padding=1, bias=True, groups=1):    
    ...
    """
    return nn.Conv2D(
        in_channels, out_channels, kernel_size=3, stride=stride, padding=padding, bias_attr=bias, groups=groups)
...
class ShuffleUnit(nn.Layer):
    def __init__(self, in_channels, out_channels, groups=3, grouped_conv=True, combine='add'):
        ...
        self.g_conv_1x1_compress = self._make_grouped_conv1x1(
            self.in_channels, self.bottleneck_channels, self.first_1x1_groups, batch_norm=True, relu=True)

        # 3x3 depthwise convolution followed by batch normalization
        self.depthwise_conv3x3 = conv3x3(
            self.bottleneck_channels, self.bottleneck_channels, stride=self.depthwise_stride, 
        ...
        self.g_conv_1x1_expand = self._make_grouped_conv1x1(
            self.bottleneck_channels, self.out_channels, self.groups, batch_norm=True, relu=False)
    ...
    def _make_grouped_conv1x1(self, in_channels, out_channels, groups,
        ...
        if len(modules) > 1:
            return nn.Sequential(*modules.items())
        else:
            return conv
...
class ShuffleNet(nn.Layer):
    ...
    def __init__(self, groups=3, in_channels=3, num_classes=100):
        ...
        self.conv1 = conv3x3(self.in_channels, self.stage_out_channels[1], stride=1)
